...
import { loadFilter, loadSizeScale, loadAspectRatio, loadCustomText } from './storage'
...
export function generateCards(formData, images = [], filterKey = null) {
  ...
  const customText = loadCustomText() || ''
  
  // Just generate a single collage card with all images
  const cards = [<CollageCard key={`collage-${filterStyle}`} images={images} year={year} filterStyle={filterStyle} sizeScale={sizeScale} aspectRatio={aspectRatio} customText={customText} />]
